...
static TM1637Display config_7seg;
...
void init_7seg() {
  // config_7seg = tm1637_init(GPIO_NUM_19, GPIO_NUM_18);
  config_7seg = TM1637Display(GPIO_NUM_19, GPIO_NUM_18, DEFAULT_BIT_DELAY);
  config_7seg.setBrightness(0x7);
  config_7seg.clear();
}
...
void set_number_7seg(uint16_t number) {
  ...
  config_7seg.showNumberDec(number);
...
void set_number_lead_dot_7seg(uint16_t number, const bool lead_zero,
  ...
  config_7seg.showNumberDecEx(number, dot_mask << 4, lead_zero);
  esp_log_level_set("gpio", gpio_log_level);
  taskEXIT_CRITICAL(&app_spinlock);
}
...
void set_segment_raw_7seg(uint8_t segment_idx, uint8_t data) {
  ...
  uint8_t segments[1];
  segments[0] = data;
  config_7seg.setSegments(segments, 1, segment_idx);
  esp_log_level_set("gpio", gpio_log_level);
  taskEXIT_CRITICAL(&app_spinlock);
}
